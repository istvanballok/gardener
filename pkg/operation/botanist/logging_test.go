	...
	Describe("#DeploySeedLogging", func() {
		It("should successfully delete the logging stack when shoot is with testing purpose", func() {
			...
			gomock.InOrder(
				...
				c.EXPECT().Delete(ctx, &networkingv1.NetworkPolicy{ObjectMeta: metav1.ObjectMeta{Name: "allow-from-prometheus-to-vali-telegraf", Namespace: seedNamespace}}),
				...
				// Delete Vali
				c.EXPECT().Delete(ctx, &networkingv1.NetworkPolicy{ObjectMeta: metav1.ObjectMeta{Name: "allow-vali", Namespace: seedNamespace}}),
				c.EXPECT().Delete(ctx, &networkingv1.NetworkPolicy{ObjectMeta: metav1.ObjectMeta{Name: "allow-to-vali", Namespace: seedNamespace}}),
		...
		It("should successfully clean up the existing Loki based deployment and deploy all of the components in the logging stack when it is enabled", func() {
			...
			gomock.InOrder(
				...
				c.EXPECT().Get(ctx, client.ObjectKey{Namespace: seedNamespace, Name: "loki-0"}, gomock.AssignableToTypeOf(&corev1.Pod{})).Return(apierrors.NewNotFound(schema.GroupResource{Resource: "Pod"}, "loki-0")),
				...
				c.EXPECT().Get(ctx, client.ObjectKey{Namespace: seedNamespace, Name: "vali-vali-0"}, gomock.AssignableToTypeOf(&corev1.PersistentVolumeClaim{})).Do(func(ctx context.Context, key client.ObjectKey, obj client.Object, opts ...client.GetOption) error {
					pvc := obj.(*corev1.PersistentVolumeClaim)
