...
`
      ...
      jsonData:
        maxLines: ` + maxLine
					...
					configMapData += `    - name: cluster-prometheus
      type: prometheus
      access: proxy
...
`
				if values.IsGardenCluster {
					configMap += `  name: plutono-datasources-2ff7a073
  namespace: some-namespace
`
					return configMap
				}
...
`
			...
			deploymentYAMLFor = func(values Values, dashboardConfigMaps []string) *appsv1.Deployment {
				...
				if values.IsGardenCluster {
					...
					dataSourceConfigMap = "plutono-datasources-2ff7a073"
				}
...
`
			...
			Context("Cluster is garden cluster", func() {
				...
				Context("Authentication is not enabled", func() {
					It("should succesfully deploy all resources", func() {
						...
						Expect(string(managedResourceSecret.Data["configmap__some-namespace__plutono-datasources-2ff7a073.yaml"])).To(Equal(dataSourceConfigMapYAMLFor(values)))
						testDashboardConfigMap(ctx, c, types.NamespacedName{Namespace: namespace, Name: "plutono-dashboards-garden-4887f387"}, 14)
						testDashboardConfigMap(ctx, c, types.NamespacedName{Namespace: namespace, Name: "plutono-dashboards-global-efcc3c92"}, 7)
						Expect(string(managedResourceSecret.Data["service__some-namespace__plutono.yaml"])).To(Equal(serviceYAMLFor(values)))
						Expect(string(managedResourceSecret.Data["ingress__some-namespace__plutono.yaml"])).To(Equal(ingressYAMLFor(values)))
						managedResourceDeployment, _, err := kubernetes.ShootCodec.UniversalDecoder().Decode(managedResourceSecret.Data["deployment__some-namespace__plutono.yaml"], nil, &appsv1.Deployment{})
						Expect(err).ToNot(HaveOccurred())
						deployment := deploymentYAMLFor(values, []string{"plutono-dashboards-garden-4887f387", "plutono-dashboards-global-efcc3c92"})
						utilruntime.Must(references.InjectAnnotations(deployment))
						Expect(deployment).To(DeepEqual(managedResourceDeployment))
					})
				})
				...
				Context("Authentication is enabled", func() {
					...
					It("should succesfully deploy all resources", func() {
						Expect(string(managedResourceSecret.Data["configmap__some-namespace__plutono-dashboard-providers-5be2bcda.yaml"])).To(Equal(providerConfigMapYAMLFor(values)))
						Expect(string(managedResourceSecret.Data["configmap__some-namespace__plutono-datasources-2ff7a073.yaml"])).To(Equal(dataSourceConfigMapYAMLFor(values)))
						testDashboardConfigMap(ctx, c, types.NamespacedName{Namespace: namespace, Name: "plutono-dashboards-garden-e9acb81e"}, 15)
						testDashboardConfigMap(ctx, c, types.NamespacedName{Namespace: namespace, Name: "plutono-dashboards-global-efcc3c92"}, 7)
						Expect(string(managedResourceSecret.Data["service__some-namespace__plutono.yaml"])).To(Equal(serviceYAMLFor(values)))
						Expect(string(managedResourceSecret.Data["ingress__some-namespace__plutono.yaml"])).To(Equal(ingressYAMLFor(values)))
						managedResourceDeployment, _, err := kubernetes.ShootCodec.UniversalDecoder().Decode(managedResourceSecret.Data["deployment__some-namespace__plutono.yaml"], nil, &appsv1.Deployment{})
						Expect(err).ToNot(HaveOccurred())
						deployment := deploymentYAMLFor(values, []string{"plutono-dashboards-garden-e9acb81e", "plutono-dashboards-global-efcc3c92"})
						utilruntime.Must(references.InjectAnnotations(deployment))
						Expect(deployment).To(DeepEqual(managedResourceDeployment))
					})
				})
		...
		Context("Cluster type is shoot", func() {
			...
			It("should succesfully deploy all resources", func() {
				...
				managedResourceDeployment, _, err := kubernetes.ShootCodec.UniversalDecoder().Decode(managedResourceSecret.Data["deployment__some-namespace__plutono.yaml"], nil, &appsv1.Deployment{})
				Expect(err).ToNot(HaveOccurred())
				deployment := deploymentYAMLFor(values, []string{"plutono-dashboards-0474caee"})
				utilruntime.Must(references.InjectAnnotations(deployment))
				Expect(deployment).To(DeepEqual(managedResourceDeployment))
			})
			...
			Context("w/ include istio, node-local-dns, mcm, ha-vpn, vpa", func() {
				...
				It("should succesfully deploy all resources", func() {
					...
					managedResourceDeployment, _, err := kubernetes.ShootCodec.UniversalDecoder().Decode(managedResourceSecret.Data["deployment__some-namespace__plutono.yaml"], nil, &appsv1.Deployment{})
					Expect(err).ToNot(HaveOccurred())
					deployment := deploymentYAMLFor(values, []string{"plutono-dashboards-1028e1dd"})
					utilruntime.Must(references.InjectAnnotations(deployment))
					Expect(deployment).To(DeepEqual(managedResourceDeployment))
				})
			})
			...
			Context("shoot is workerless", func() {
				...
				It("should succesfully deploy all resources", func() {
					...
					managedResourceDeployment, _, err := kubernetes.ShootCodec.UniversalDecoder().Decode(managedResourceSecret.Data["deployment__some-namespace__plutono.yaml"], nil, &appsv1.Deployment{})
					Expect(err).ToNot(HaveOccurred())
					deployment := deploymentYAMLFor(values, []string{"plutono-dashboards-42b1243d"})
					utilruntime.Must(references.InjectAnnotations(deployment))
					Expect(deployment).To(DeepEqual(managedResourceDeployment))
				})
			})
