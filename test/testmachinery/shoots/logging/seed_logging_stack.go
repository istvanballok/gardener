...
var _ = ginkgo.Describe("Seed logging testing", func() {
	...
	framework.CBeforeEach(func(ctx context.Context) {
		...
		framework.ExpectNoError(f.SeedClient.Client().Get(ctx, types.NamespacedName{Namespace: v1beta1constants.GardenNamespace, Name: valiName}, gardenValiService))
		...
		framework.ExpectNoError(f.SeedClient.Client().Get(ctx, types.NamespacedName{Namespace: f.ShootSeedNamespace(), Name: valiName}, shootValiService))
	...
	f.Beta().Serial().CIt("should get container logs from vali for all namespaces", func(ctx context.Context) {
		...
		framework.ExpectNoError(create(ctx, f.ShootClient.Client(), prepareGardenValiService(gardenValiService)))
		...
		framework.ExpectNoError(create(ctx, f.ShootClient.Client(), prepareShootValiService(shootValiService, shootValiName, shootValiLabels)))
		...
		for i := 0; i < numberOfSimulatedClusters; i++ {
			...
			ginkgo.By(fmt.Sprintf("Deploy the vali service in namespace %s", shootNamespace.Name))
			valiShootService := getValiShootService(i)
			framework.ExpectNoError(create(ctx, f.ShootClient.Client(), valiShootService))
		...
		ginkgo.By("Verify vali received all operator's logs from operator's logger application for all shoot namespaces")
		framework.ExpectNoError(WaitUntilValiReceivesLogs(ctx, 30*time.Second, f, shootValiLabels, operatorID, v1beta1constants.GardenNamespace, "pod_name", operatorLoggerRegex, (logsCount*numberOfSimulatedClusters)/2, numberOfSimulatedClusters/2, f.ShootClient))
		ginkgo.By("Verify vali received all operator's logs from user's logger application for all shoot namespaces")
		framework.ExpectNoError(WaitUntilValiReceivesLogs(ctx, 30*time.Second, f, shootValiLabels, operatorID, v1beta1constants.GardenNamespace, "pod_name", userLoggerRegex, (logsCount*numberOfSimulatedClusters)/2, numberOfSimulatedClusters/2, f.ShootClient))
		ginkgo.By("Verify vali received user logger application logs for all shoot namespaces")
		framework.ExpectNoError(WaitUntilValiReceivesLogs(ctx, 30*time.Second, f, shootValiLabels, userID, v1beta1constants.GardenNamespace, "pod_name", userLoggerRegex, (logsCount*numberOfSimulatedClusters)/2, numberOfSimulatedClusters/2, f.ShootClient))
		ginkgo.By("Verify vali didn't get the logs from the operator's application as user's logs for all shoot namespaces")
		framework.ExpectNoError(WaitUntilValiReceivesLogs(ctx, 30*time.Second, f, shootValiLabels, userID, v1beta1constants.GardenNamespace, "pod_name", operatorLoggerRegex, 0, 0, f.ShootClient))

		ginkgo.By("Verify vali received logger application logs for garden namespace")
		framework.ExpectNoError(WaitUntilValiReceivesLogs(ctx, 30*time.Second, f, gardenValiLabels, "", v1beta1constants.GardenNamespace, "pod_name", operatorLoggerRegex, (logsCount*numberOfSimulatedClusters)/2, numberOfSimulatedClusters/2, f.ShootClient))
		ginkgo.By("Verify vali received user application logs for garden namespace")
		framework.ExpectNoError(WaitUntilValiReceivesLogs(ctx, 30*time.Second, f, gardenValiLabels, "", v1beta1constants.GardenNamespace, "pod_name", userLoggerRegex, (logsCount*numberOfSimulatedClusters)/2, numberOfSimulatedClusters/2, f.ShootClient))
		...
		for i := 0; i < numberOfSimulatedClusters; i++ {
			...
			valiShootService := getValiShootService(i)
			framework.ExpectNoError(kubernetesutils.DeleteObject(ctx, f.ShootClient.Client(), valiShootService))
...
func prepareGardenValiService(service *corev1.Service) *corev1.Service {
...
func prepareShootValiService(shootValiService *corev1.Service, name string, selector map[string]string) *corev1.Service {
	shootValiService.Name = name // "vali-shoots"
	shootValiService.Namespace = v1beta1constants.GardenNamespace
	shootValiService.Spec.Selector = selector
	shootValiService.Spec.ClusterIP = ""
	shootValiService.Spec.ClusterIPs = nil
	return shootValiService
}
