...
var _ = ginkgo.Describe("Seed logging testing", func() {
	...
	framework.CBeforeEach(func(ctx context.Context) {
		...
		framework.ExpectNoError(f.SeedClient.Client().Get(ctx, types.NamespacedName{Namespace: v1beta1constants.GardenNamespace, Name: valiName}, gardenLokiService))
		...
		framework.ExpectNoError(f.SeedClient.Client().Get(ctx, types.NamespacedName{Namespace: f.ShootSeedNamespace(), Name: valiName}, shootLokiService))
	...
	f.Beta().Serial().CIt("should get container logs from vali for all namespaces", func(ctx context.Context) {
		...
		for i := 0; i < numberOfSimulatedClusters; i++ {
			...
			valiShootService := getLokiShootService(i)
...
func prepareShootLokiService(shootLokiService *corev1.Service, name string, selector map[string]string) *corev1.Service {
	shootLokiService.Name = name // "vali-shoots"
